...
TITLES = {
    ...
    "Raft选举与日志复制把共识落到实现": "Raft elections and log replication bring consensus down to implementation",
    "Saga把长事务拆成补偿链，代价是失去隔离性": "Saga splits long transactions into compensation chains at the cost of isolation",
    ...
    "API 契约定义输入输出和演进边界": "API contracts define inputs, outputs and evolution boundaries",
    "HTTP语义的分层：方法、状态码与头部的契约角色": "HTTP semantics in layers: the contract roles of methods, status codes and headers",
    ...
    "微服务拆分的判据：从团队认知到数据边界": "Criteria for splitting microservices: from team cognition to data boundaries",
    "服务发现与负载均衡决定请求落到哪里": "Service discovery and load balancing decide where requests land",
