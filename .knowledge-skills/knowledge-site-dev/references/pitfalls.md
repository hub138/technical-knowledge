...
- **正解**：测试实例的 `KNOWLEDGE_DATA_HOME` 与 `TMPDIR` 指向仓库内已忽略的 `.agent/tmp/` 独立子目录；使用真实文章，测试访问记录保持隔离。
...
- 本站存在轮询请求，导航使用 `domcontentloaded`，随后等待目标文章标题、正文、字体与图形完成渲染。懒加载图片先滚动到对应位置，再等待 `complete` 并验证 `naturalWidth > 0`。
...
- **按元素分配阅读层级**：表格承载文章解释，必须继承正文字号。token 名称需要核对最终值；验收逐项测量普通文字、列表、引用、单元格与代码，不能只读取 `.reader-body` 或首个 `p`。
- **保留浏览器原生布局**：表头、表体共享一个表格计算列宽，外层容器负责滚动。检查每列左右边界、长词换行和最后一列可达性，页面整体宽度正常仍需要完成这些检查。
- **覆盖完整阅读过程**：代表文章要包括表格、代码、引用、图片与长标题；全页截图检查整体关系，局部截图检查文字。截图前验证文章身份、正文加载和主题状态，零元素不能算通过。
- **将历史问题转成检查项目**：目录遮挡对应固定区域碰撞，正文后空白对应隐藏元素占位，图片裁切对应容器与内容尺寸，语言切换截断对应实际文本宽度。每次修改都要检查同组件的其他状态和使用位置。
- **规范必须连接执行入口**：`scripts/note_verify_shots.py` 负责文章文字与表格，`scripts/check_all.py` 在完整检查中调用它。保留修改前后的 JSON 和截图，检查失败返回非零状态；服务测试与文章排版分别报告。
- **修复共用规则**：先检查基础样式、媒体查询和后续覆盖，再在规则来源修改；禁止不断追加覆盖。开发、内容治理、配图检查共同发现阅读问题，由站点开发维护共用样式。
