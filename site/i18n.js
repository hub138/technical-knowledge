...
"本文目录": { en: "On this page" },
    ...
    /* 【rt25·标题批 2/7】缺陷分析域 60 篇。案例标题按
       ...
       双段结构，方便与中文原题对照。 */
    "兼容与性能——滞后的依赖与放大的日志": { en: "Case 7: compatibility and performance — lagging dependencies and amplified logs" },
    "升级把前提带走了——依赖行为变更引发的下游失效": { en: "Case 43: the upgrade took its premises away — downstream breakage from dependency behaviour changes" },
    ...
    "公告栏没查门禁——Redis 的 ACL 键名泄露与解析崩溃": { en: "Case 26: the bulletin board skipped the gate — Redis ACL key-name leaks and parser crashes" },
    "安全两例——认证绕过与带漏洞依赖": { en: "Case 5: two security cases — auth bypass and a vulnerable dependency" },
    "投毒不是写错，是写给你看——依赖投毒的三个真实剧本": { en: "Case 44: poisoning is not a bug, it is written for you — three real dependency-poisoning scripts" },
    ...
    "回收之后还在飞——跨线程复用可回收对象": { en: "Case 31: still flying after recycling — cross-thread reuse of recyclable objects" },
    "取到了已经不存在的那个——估算器的读取竞态": { en: "Case 32: fetched the one that no longer exists — a read race in the estimator" },
    "并发两面——写回调卡死与心跳竞态": { en: "Case 3: two faces of concurrency — a stuck write callback and a heartbeat race" },
    "关闭之后还有人在用——生命周期的时序竞态": { en: "Case 20: someone still using it after close — a lifecycle timing race" },
    "在回调里改自己——清理路径的重入与回收后复用": { en: "Case 19: mutating itself from a callback — re-entry on the cleanup path and use after recycle" },
    "一方取消，旁观者陪葬——RocksDB 状态加载的并发陷阱": { en: "Case 14: one side cancels, bystanders go down too — a concurrency trap in RocksDB state loading" },
    "谁在回答谁——Pulsar 连接复用与去重的键不完整": { en: "Case 40: who is answering whom — Pulsar connection reuse and incomplete dedup keys" },
    ...
    "锁自己变成了瓶颈——争用背后的临界区错配": { en: "Case 37: the lock became the bottleneck — critical-section mismatch behind the contention" },
    "库自带了一支军队——OpenBLAS 的隐式线程": { en: "Case 25: the library shipped an army — OpenBLAS and its hidden threads" },
    "并行度被自己折叠——repartition 的 2 的幂倾斜": { en: "Case 24: parallelism folded by itself — power-of-two skew in repartition" },
    "估算不等于算术——按大小推条目的错算": { en: "Case 33: estimation is not arithmetic — miscounting entries from sizes" },
    "身份在改写中丢失——RLS 子查询与视图 DEFAULT 的静默失真": { en: "Case 39: identity lost in rewriting — silent distortion from RLS subqueries and view DEFAULTs" },
    "省下的句柄与多出的删除——一次优化引入的回归": { en: "Case 34: handles saved, deletions gained — a regression introduced by an optimisation" },
    "中间缺了一块——跨地域复制的 ack 空洞": { en: "Case 30: a hole in the middle — ack gaps in cross-region replication" },
    "数字越过了边界——Redis 里三次截断导致的崩溃": { en: "Case 9: numbers across the boundary — three truncations that crashed Redis" },
    "离开的人还占着座位——机架信息的陈旧映射": { en: "Case 29: the departed still hold seats — stale rack-info mappings" },
    "静默错算——Spark SQL Union 的别名陷阱": { en: "Case 2: silent miscalculation — the alias trap in Spark SQL unions" },
    "键序与列名——Spark SQL 又两种静默错算": { en: "Case 13: key order and column names — two more silent miscounts in Spark SQL" },
    "空集合被当成了\"全部已确认\"——游标跳过的位置": { en: "Case 18: an empty set read as \"all confirmed\" — positions the cursor skipped" },
    "数据被自己人删掉——entry log 头里的假账": { en: "Case 16: data deleted by its own side — false books in the entry-log header" },
    "类型说谎时——静默删数据的 TTL 与算错的比较": { en: "Case 10: when types lie — a TTL that silently deletes data and a miscounted comparison" },
    "确认了却没确认——Pulsar 消息语义的三种失真": { en: "Case 41: confirmed but not confirmed — three distortions of Pulsar message semantics" },
    "备份里的假账——恢复路径的静默损坏": { en: "Case 42: false books in the backup — silent corruption on the restore path" },
    ...
    "垃圾回收线程的终止——BookKeeper 磁盘写满": { en: "Case 1: termination of the garbage-collection thread — BookKeeper with a full disk" },
  "写被自己的回调堵住——挂起的 add 回调": { en: "Case 36: writes blocked by their own callback — a stuck add callback" },
    "异常路径上没有完成——永不兑现的 future": { en: "Case 21: never completed on the error path — futures that never resolve" },
    "一次抖动变成一次重算——SASL 拉取没有重试": { en: "Case 23: one hiccup becomes a recompute — SASL fetch without retries" },
    "时间也是输入——夏令时让序列生成越界": { en: "Case 22: time is input too — daylight saving pushes sequence generation out of range" },
    "承诺没有兑现——Pulsar 里六个永不返回的请求": { en: "Case 8: promises unkept — six Pulsar requests that never return" },
    "上游修好了，你这边没有——一个发行分支的移植缺口": { en: "Case 11: fixed upstream, not on your side — a backport gap on a release branch" },
    "调度线程的永久等待——广播写锁不释放": { en: "Case 12: the scheduler waits forever — a broadcast write lock never released" },
    ...
    "容量与泄漏——连接泄漏和 SST 堆积": { en: "Case 6: capacity and leaks — connection leaks and SST pile-up" },
    "删除列表里的幽灵——永不收敛的待删账本": { en: "Case 17: ghosts in the deletion list — a deletion ledger that never converges" },
    "资源泄漏的三段式排查法：预筛确认与归因": { en: "A three-stage approach to resource leaks: pre-screening, confirmation, and attribution" },
    "后写的覆盖了先写的——策略层级的覆盖顺序": { en: "Case 35: later writes override earlier ones — override order across policy tiers" },
    "没有写下来的版本——默认值里的迁移债": { en: "Case 38: the version that was never written down — migration debt hidden in defaults" },
    "错误最晚在哪一刻暴露——Imputer 与公平调度的两种延迟": { en: "Case 15: the latest moment an error can surface — two delayed failures, Imputer and fair scheduling" },
    "配置不生效——recover 速率与重复日志": { en: "Case 4: configuration that never took effect — recovery rate and duplicated logs" },
...
"带证据链的评估结论与回归清单；当前是设计约定与静态原型": { en: "Evaluation conclusions with evidence chains and regression checklists; currently a design convention and a static prototype" },
    ...
    "按天去重访问": { en: "Daily unique visits" },
    "同一天同一访客只算一次": { en: "One visitor per day counts once" },
    "总停留时长": { en: "Total dwell time" },
    "读者在页面上的累计停留": { en: "Cumulative time readers stayed on pages" },
    "天数": { en: "Days" },
    "停留": { en: "Dwell" },
    "秒": { en: "s" },
    "分钟": { en: "m" },
