...
derived:
  ...
  - "[[工程知识/软件构建与质量/测试与交付/测试策略从风险选择证据]]"
---

# AI 研发迭代框架案例证据

本页从整理前的三份工作原稿中保留可复核的案例矩阵和失败事实：代码修复候选、MySQL 用例复现候选，以及 `mr_six v0.2.27 / Bug #119991` 的迭代运行记录。具体机器路径、端口、模型默认值、重复提示词和未执行待办不构成长期证据，已移除。
...
当时 `mutate` 阶段的契约是只输出结构化 operations，由引擎统一应用、验证和回滚。模型却尝试直接通过命令/写文件修改磁盘，约 7.8 分钟后被执行引擎拦截并失败。
...
- AI 与框架职责、事件与投影：[[工程知识/AI系统/安全与治理/执行证据必须独立于AI结论]]。
- 代码理解、假设、修改和验证闭环：[[工程知识/AI系统/Agent与工作流/AI辅助研发必须形成证据闭环]]。
- 结构/符号索引与路径证明：[[工程知识/AI系统/Agent与工作流/代码智能体需要结构索引、任务探索与运行证据]]。
- 测试层次、环境阻断和回归资产：[[工程知识/软件构建与质量/测试与交付/测试策略从风险选择证据]]。
- 进程退出、孤儿和资源清理：[[工程知识/后端与分布式系统/任务与并发/任务生命周期必须覆盖进程、日志、超时与清理]]。
