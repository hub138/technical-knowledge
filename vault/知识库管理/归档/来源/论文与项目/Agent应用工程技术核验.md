...
- 工具描述是模型看到的 API 约定。工具数量增长后，应做命名空间、候选集过滤或动态加载，而不是把全部 schema 永久塞入上下文。
...
| 名词 | 处理方式 | 原因 |
| --- | --- | --- |
| Prompt engineering | 降为“上下文与约定设计”的子主题 | 只优化文本不能覆盖状态、工具、数据、权限和评测 |
