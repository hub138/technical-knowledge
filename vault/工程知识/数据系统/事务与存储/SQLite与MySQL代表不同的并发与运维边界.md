...
SQLite 和 MySQL 都提供 SQL、事务和索引，但它们解决的系统问题不同：SQLite 是嵌入应用的本地数据库库，MySQL 是通过服务进程向多个客户端提供共享数据的客户端/服务器数据库。选型的关键不是背“适合小项目/大项目”，而是判断数据位置、写入并发、故障边界、运维责任和未来迁移成本。
...
两者都把表和索引组织成页，并通过日志/锁保证事务的原子性与崩溃恢复。索引减少扫描，却会增加写放大、空间和维护成本；事务越大，锁持有时间、日志量和恢复时间通常越高。无论选哪种数据库，都应先定义：
