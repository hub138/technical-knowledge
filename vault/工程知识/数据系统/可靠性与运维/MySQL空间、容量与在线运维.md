...
`GRANT` 会更新权限表；现代 MySQL 通常会立即生效，不应把 `FLUSH PRIVILEGES` 当作每次授权后的固定动作。兼容产品（例如 TDSQL/TXSQL）要用最小复现记录与原版 MySQL 的差异，不能只依据产品宣传或旧笔记。

## 关联页面

- [[工程知识/数据系统/查询与索引/B+Tree索引为访问路径服务]]
