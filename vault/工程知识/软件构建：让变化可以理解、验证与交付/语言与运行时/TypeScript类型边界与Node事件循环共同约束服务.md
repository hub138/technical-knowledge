...
TypeScript 提供静态检查和可维护的接口约定，Node.js 提供事件循环、异步 I/O 和服务端运行时；两者都不能自动验证网络输入、数据库数据或外部工具结果。工程边界应是：**类型约束内部程序，runtime schema 校验外部世界，超时/取消/背压约束执行。**
...
- 生产使用受支持的 LTS 线，具体状态以 [Node.js release 页面](https://nodejs.org/en/about/previous-releases) 为准；不要长期固定在旧课版本。
...
- 类型检查只证明静态约定，不替代 runtime schema、集成测试和权限测试。
- 单测覆盖纯逻辑；接口约定测试覆盖 HTTP/消息/schema；集成测试覆盖真实数据库与生命周期。
...
关联：[[工程知识/后端系统：在并发、失败与变化中维持服务/服务设计/HTTP请求穿过浏览器、网络与服务端边界]]、[[工程知识/后端系统：在并发、失败与变化中维持服务/任务与并发/任务生命周期必须覆盖进程、日志、超时与清理]]、[[工程知识/后端系统：在并发、失败与变化中维持服务/分布式可靠性/部分失败决定分布式系统的设计]]、[[工程知识/软件构建：让变化可以理解、验证与交付/测试与交付/测试策略从风险选择证据]]。

## 验证

1. 类型擦除核验：在运行时向标注了领域类型的入口传入字段缺失或类型不符的载荷，预期错误出现在边界校验处；缺少校验时错误会出现在远离入口的位置。
2. 调度延迟度量：在请求处理路径上插入一段同步计算，记录 event-loop delay 与延迟分位数，预期其后所有请求一起被推迟，推迟幅度与计算耗时可对照。
3. 流式背压：构造生产速度高于消费速度的 stream，预期写入方被背压阻塞、内存保持平稳，忽略 backpressure 的实现会出现缓冲持续增长。
4. 生命周期核验：对 fire-and-forget promise 的路径检查 owner、取消、错误接收与持久状态；取消请求后预期相关工作停止而不是继续执行。
5. 断言锚点：边界处有 runtime schema 校验，event-loop delay 进入性能记录，stream 背压与取消行为经过对照测试。

## 效果与代价

用类型约束内部结构、用 schema 校验外部输入、用取消与背压约束执行，换来的是编译期就能拦下的一部分接口误用，以及边界处数据形状的显式确认。代价是运行时校验的额外开销与一套需要跟随上游版本维护的生成流程。类型检查省下的是评审与回归的人力，代价出现在每次边界变更都要同步 schema 与生成代码。

## 机制与本质

TypeScript 与 Node.js 共同约束服务的机制层：类型只在编译期存在，编译之后被擦除，运行时收到的仍是未被验证的字节；事件循环把 I/O 交给 libuv 与线程池，回调在单线程上依次执行，任何一处同步计算都会推迟其后所有请求。静态约束到不了外部世界，运行时事实又不受类型影响，两个边界分别由类型检查和 schema 校验负责。

## 要解决的问题

编译通过的服务把外部返回的 JSON 直接标注为领域类型，字段缺失或被替换之后错误出现在远离边界的位置；一个 CPU 密集的同步调用让并发能力从数万掉到数十；无界并发向下游发起请求，最先失效的是被依赖的一方。本篇回答：类型在运行时留下什么、网络与数据库输入在哪里校验、事件循环上的工作如何分类、取消与背压由谁提供、以及模块解析与依赖升级要核对哪些项。
