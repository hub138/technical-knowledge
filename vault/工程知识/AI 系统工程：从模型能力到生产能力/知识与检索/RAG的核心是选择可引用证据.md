...
editorial_note: "清三处二分对照（摘要句保留信号词、核心定义句改证据系统直接陈述、小节标题），命中3到0，密度0.35达标维持链路图"
---

# RAG 的核心是选择可引用证据

RAG 要解决的是模型参数无法及时、可追溯地覆盖外部事实的问题。核心是按问题、身份和时间选择能支持结论的证据，把更多文本塞进上下文解决不了这件事；边界是召回命中不等于模型使用正确，必须能拒答、引用和处理冲突版本。最小实验是用单跳、数字、无答案和撤权样本分别测召回与引用支持。
RAG 是一个证据系统：针对当前问题、身份和时间，从外部知识中选择能够支持结论的材料，并让答案与这些材料建立可检查的关系。"向量数据库 + 大模型"的印象只覆盖了候选生成与生成两段；解析、权限、版本、排序、证据覆盖和拒答共同决定系统质量。
...
切块需要尊重标题、表格、代码、列表和父子关系。固定字符切块虽然简单，却容易把条件与结论、函数签名与实现、表头与数据分开。对代码应保留符号、文件和引用关系；对表格应保留表头与行列语义；对政策应保留生效时间和适用主体。
...
RAG 的机制层：模型的参数里只有训练截止前的通用模式，外部事实必须由当前可访问的材料补充，而材料能否出现在答案里取决于它经过解析、过滤、召回与重排之后是否进入模型可见的位置。相似度只说明文本接近，能否支持结论取决于权限、版本与时间过滤之后的证据集合，这两件事的判断依据不同。

验证的锚点：检索结论要与引用对账——预写每个答案都能指回原文片段，指不回或只能指到模型解释时，与预期对不上，该证据不可引用。

## 要解决的问题

模型参数无法及时覆盖外部事实，检索到的内容也无法证明它支持当前结论，团队只能看到回答质量不稳定而找不到原因。本篇回答：按问题与身份与时间选择证据的依据是什么、召回命中与正确使用之间的差距在哪里、无法回答时怎样拒答，以及处理同一事实的多个版本时按什么顺序判定。
