...
editorial_note: "补decode读写回路数据流图与vLLM论文来源，清抽象主语与模糊判据，tags去重且双轴命中保持"
---
...
**四条优化路线**。
- MQA/GQA（多查询/分组查询注意力）：KV 头数从“每 Q 头一个”降到“全局一个”或“每组一个”。KV 头数是公式里的乘数，LLaMA-2 7B 用 GQA 把 32 个头归成 8 组，KV 显存按头数比例直降（收益按架构配置表核对，不按经验数字）。代价：Q 与 KV 的信息交互减少，精度有可测损失（中等任务小、难任务可测）。现代模型架构默认 GQA（LLaMA-3、Qwen 系）。
...
**上下文工程视角的省钱**。公式里的序列长度不只是模型能力问题，还是工程输入：系统提示占位、工具结果回填、检索片段拼接都把序列长度推大。做上下文工程（见 [[工程知识/AI 系统工程：从模型能力到生产能力/模型与上下文/上下文工程是在有限预算内构造决策现场.md]]）时，你在固定显存预算下做内容取舍——预算表发给 Agent 框架的调用方，让“多塞一个检索片段”成为有价格的操作。
...
```mermaid
flowchart TD
    A["decode 第 t 步"] --> B["读全部历史 KV<br/>t 越大读得越多（GB 级）"]
    B --> C["注意力计算<br/>受显存带宽约束"]
    C --> D["写 1 条新 KV<br/>显存占用 +1"]
    D -->|"t+1 步"| A
```

图中回路解释长上下文的双重贵：每步读的量随 t 线性涨（速度账），每步写的量累积占显存（容量账），同一状态两个账同时恶化。

## 背景与代价

思想背景：KV 缓存随 2020s 初的开源对话模型服务化出现（vLLM 之前各框架的 KV 管理粗放，静态预分配）。PagedAttention（vLLM，2023）是分水岭：把 OS 分页思想搬进 KV 管理，论文实测碎片浪费从约 80% 降到 4% 以下、同负载吞吐提升 2-4 倍。GQA（2023，Google）从架构侧把 KV 头数变成设计参数，此后开源模型默认 GQA。FlashAttention（2022-2023）从计算侧解决 attention 的显存访问效率（IO 感知精确注意力），与 KV 缓存管理正交但互补。
...
- **PagedAttention 碎片实验**：vLLM 的 PagedAttention 与静态预分配（旧框架或配置）对照，同负载（序列长度分布有方差）跑吞吐，预期：分页版吞吐提升到 2 倍上下（vLLM 论文实测区间），长度分布越参差收益越大；分布无方差时两者接近。
