...
editorial_note: "清开篇与验证段的条件句堆叠，标题保护项不动"
---

# 先用工作流，只有路径无法预先枚举时才使用 Agent

这条选择规则处理固定流程被交给模型的场景——那会让成本、不可预测性和审计难度同时上升。判断做三步：路径是否能在设计时枚举、错误动作是否可逆、是否有外部验证。中间发现会改变下一步且无法经济枚举时，自治循环才可能产生净收益。
“使用大模型”和“构建 Agent”是两个决定。一次结构化抽取、固定步骤的文档处理和带模型节点的审批流都可以使用大模型，这些场景不需要让模型决定下一步。任务路径取决于中间发现、且无法经济地预先枚举时，自治循环才产生净价值。
...
下一步始终相同时，使用工作流。模型控制循环的适用场景是“观察结果决定新的动作”。
...
![routing 工作流：输入先经分类器判定类型，再分发到各自专用的下游处理路径](/static/figures/workflow-routing.png)

![并行化工作流：任务拆开同时跑，结果由程序聚合](/static/figures/workflow-parallelization.png)
不要把“多 Agent”当作一种默认模式。划分成多个 Agent 只有在上下文隔离、权限隔离、并行执行或独立评价确实有价值时才合理；否则它只是增加通信损失、状态同步和评测难度。
...
验证的锚点：自治收益要与总成本对账——预写减少的人工点击折成多少成本、新增的失败重试行多少，两头对不上就是负收益。

## 验证设计

评测时至少保存：选择了哪条路径、每步为什么发生、工具是否必要、是否错过停止、失败后是否能恢复。同一任务分别用固定工作流和 Agent 实现，比较任务成功率、P95 延迟、费用、人工介入和不可恢复错误。Agent 在真实任务分布上提供稳定净收益，才提高自治程度；收益不稳定就停在低自治档位。

相关：[[工程知识/AI 系统工程：从模型能力到生产能力/Agent与工作流/模型负责判断，运行时负责执行语义]] · [[工程知识/AI 系统工程：从模型能力到生产能力/质量与运营/Agent评测必须覆盖轨迹而不只看最终答案]]

## 机制与本质

工作流与 Agent 的机制层：控制权放在程序里还是放在模型里，决定了路径在设计阶段还是运行阶段确定。程序控制的路径可以枚举、可以覆盖测试、成本有上界；模型控制路径适合中间发现会改变下一步的场景，代价是路径不可预先枚举、成本必须由预算限制、失败恢复还要带回模型决策时的上下文。

## 要解决的问题

一条在设计时就可以写定的流程被交给自治循环执行之后，成本上升、执行路径每次不同、审计时说不清当时做过哪些动作。本篇回答：路径能否在设计阶段枚举、错误动作能否撤销、结果是否有外部可验证的判据，这三项怎样决定用固定的工作流还是用自治循环。
