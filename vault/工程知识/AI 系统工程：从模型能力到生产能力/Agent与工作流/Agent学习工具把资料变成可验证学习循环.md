...
editorial_note: "清开篇二分对照、三处套话与条件句，标题保护项保留"
---

# 知识库的学习闭环：从来源到迁移

一个知识库的价值体现在：人或 Agent 能从一个真实问题出发，找到有证据的解释，完成可观察的练习，并把结果迁移到新的问题。图、检索、课程和记忆都只是这条路径上的不同表达与执行方式。

## 合并后的抽象

资料从来源登记起步，经过提炼、检索、练习与评测，最终以新结论的形式回到主题知识。下图给出各环节的先后与合并位置：
...
一轮完整学习分七步：选定主题，读主题页与近期变更，把问题提成可验证的形式，检索带来源的证据，用证据支撑图、解释或练习，核验正确性与成本，通过才把新结论合并回主题页。
...
三条表达路径的产物都要能回到来源页核验，这是三类方式共享的收束点。
