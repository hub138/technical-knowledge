...
| 正在做跨系统自动化 | 工具约定 → MCP 与 A2A → 幂等与补偿 → 人工审批 → 审计 |
| 正在做系统选型 | [[工程知识/AI 系统工程：从模型能力到生产能力/生态与选型/Agent系统选型不是框架排名]] → [[工程知识/AI 系统工程：从模型能力到生产能力/生态与选型/AI技术动态]] |
...
验证的锚点：完成判定要绑定五件证据——预写命令、退出码、diff、测试报告与原问题复现齐备才算完成，缺项与预期对不上即判定未完成。

## 验证

1. 完成判定：以自动修复测试为例，确认命令、退出码、diff、测试报告与原问题复现五项都齐备，缺一项即判定未完成。
2. 六类对象核对：逐个确认任务状态、工具约定、权限、隔离、证据与观测都有归属，没有留在模型自行记忆里。
3. 越权与副作用：确认模型提出的动作受运行时权限与隔离约束，未授权动作在执行前被拦下。

## 机制与本质

Agent 应用的机制层：模型输出的是候选理解与动作，这些候选是否被允许执行、以什么状态执行、执行后如何证明完成，取决于运行时持有的状态、权限与证据。把模型的能力放在可约束、可恢复、可验证的系统里，任务完成的判定才能脱离模型自述，由工具结果与记录共同支持。

## 要解决的问题

模型在演示用例上表现良好，接入真实数据、真实权限和真实写入操作之后，失败形态无法复现，完成状态也无法核对。本篇回答：模型侧负责提出什么、运行时侧负责保证什么、一次任务的完成状态由哪些记录共同判定，以及副作用与重试怎样协同。
