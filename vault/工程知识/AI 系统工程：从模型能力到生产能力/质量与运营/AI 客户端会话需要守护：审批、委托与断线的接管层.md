...
| 文件读写 | 读文件、写文件的请求发出去，对端不应答 → 挂死 | 读写本身委托给另一个连接执行，投递失败却静默丢弃 |
...
用户终端 / IDE
      ...
      ▼
┌─────────────── AI 客户端会话 ───────────────┐
│  模型自主循环 → 触发审批 / 选项题 / 文件读写   │
└──────────────────┬──────────────────────────┘
                   │ 中断到达（approval.wait / 委托请求）
                   ▼
        ┌──────────────────────────┐
        │   常驻守护（独立进程）      │
        │  · 审批策略：自动签 / 拒    │
...
**文件读写委托代执行**——这是最隐蔽的一类。客户端的读文件、写文件不是本地直接做的，而是把请求**委托**给一个对端连接执行（fs/read_text_file、fs/write_text_file）。守护作为那个对端应答，读写才秒回。如果守护不在，请求发给一个尸体连接，就永久挂死。这条委托链路也是为什么挂死时「读写全挂、命令执行不挂」——命令是后台本地跑的，不走窗口；读写走窗口。

**断线与卡死自愈**——两层兜底：

- 断线自愈：连接断了按三级恢复提示词逐级尝试重连续跑，而不是直接放弃会话。
- 卡死自救：给每次工具调用一个「无结果时限」（例如 15 分钟无任何输出），超时就 kill 掉这次调用并 resume 续跑。这对应 [[工程知识/后端系统：在并发、失败与变化中维持服务/分布式可靠性/超时的层次设计：连接、读写与整链路的预算分配]]——每一层调用都要有自己的死期。
...
3. **再接文件读写委托**：这一类不显眼但一旦挂死最难查，守护作为对端应答即可根治。
...
相关（契约层）：[[工程知识/AI 系统工程：从模型能力到生产能力/质量与运营/可观测性必须能重建一次决策]] —— 守护的每一次代答都应进日志可重建
