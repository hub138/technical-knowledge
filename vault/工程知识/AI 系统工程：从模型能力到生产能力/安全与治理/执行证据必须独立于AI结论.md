...
- AI 辅助研发：[[工程知识/AI 系统工程：从模型能力到生产能力/Agent与工作流/AI辅助研发必须形成证据闭环]] 负责把本原则落实到排障、修复和验证闭环。
...
- MCP：[[工程知识/AI 系统工程：从模型能力到生产能力/生态与选型/MCP接入必须验证协议、身份与工具契约]] 只解决能力连接，不替代事实门禁和业务授权。
...
| 主进程死亡后状态永远 running | 超时和孤儿检测必须有进程外 observer/lease | [[工程知识/后端系统：在并发、失败与变化中维持服务/任务与并发/任务生命周期必须覆盖进程、日志、超时与清理]] |
