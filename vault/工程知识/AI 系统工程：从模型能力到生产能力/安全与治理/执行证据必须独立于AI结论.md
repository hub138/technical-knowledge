...
最后一行最隐蔽：**一次重试失败，把之前已经确认的成功也抹掉了**。出错的环节在状态模型——它只有一个可被覆盖的字段，没有"哪个 attempt 产生了哪个事实"的概念。
...
- AI 辅助研发：[[工程知识/AI 系统工程：从模型能力到生产能力/Agent与工作流/AI辅助研发必须形成证据闭环]] 负责把本原则贯穿到排障、修复和验证的每一步。
