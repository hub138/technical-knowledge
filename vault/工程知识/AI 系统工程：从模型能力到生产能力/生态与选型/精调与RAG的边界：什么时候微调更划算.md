...
思想背景：RAG（2020，Lewis et al.）诞生于“参数化知识（权重）与非参数化知识（检索）互补”的洞察，2023 年随企业应用潮成为知识问答标配。精调比 RAG 老（BERT 时代就有 domain adaptation 传统），LoRA（2021）把精调门槛降一个数量级（消费级卡可训），2023-2024 企业精调潮。两条路线的边界讨论是 2024 年后的热点（业界共识逐渐收敛：知识用 RAG、行为用精调、两者组合）。
