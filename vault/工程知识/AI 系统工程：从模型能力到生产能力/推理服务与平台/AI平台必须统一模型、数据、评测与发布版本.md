...
editorial_note: "L89只有才能条件句改收益门槛独立陈述，命中1到0；本轮累计：prompt等英文4处汉化，补效果与代价节（收益三条编号列表+代价与简化形态）"
---

# AI 平台必须统一模型、数据、评测与发布版本

AI 平台要解决的是模型、索引、提示词、工具和评测各自发布，最后无法解释线上结果来自哪一组输入。核心机制是把这些对象绑定成可重放发布单元，再通过灰度、观测和回滚改变版本；收益是问题可定位、改动可比较，代价是注册、存储和发布门禁复杂度增加。
AI 服务的一个“版本”从来不只是模型文件。实际行为由模型、推理参数、提示词、工具、检索索引、业务代码和安全策略共同决定。只记录镜像标签，无法解释回归，也无法可靠回滚。
...
Kubernetes 能调度 GPU、维护副本和执行滚动发布，但不了解模型质量、KV 缓存容量、token SLO 或提示词兼容性。Ray Serve、vLLM、TensorRT-LLM 等可以承担部分服务与运行时能力，也仍需应用定义版本、评测和回滚门禁。
...
1. 回归定位：对比两次发布单元的差异项，排查范围从全量缩到改动对象。
2. 回滚：还原完整对象组，提示词或索引单独出错时不再连带重启全部组件。
3. 审计：回答“当时线上跑的是哪一组版本”。

代价由平台侧承担：注册中心要为模型快照、索引版本和评测结果提供不可变存储，发布门禁要在每次上线前核对全部对象的兼容性，这套基础设施对小团队是固定开销。收益要过发布频率这道门槛：发布频率达到一定次数后，定位与回滚省下的时间才盖得过维护成本。发布频率低、对象组成简单的服务，可以先用“镜像标签加版本清单文件”的简化形态，保留对账能力，暂缓自建注册中心。
...
- 日志、提示词和检索内容按敏感度采集，不能为了可观测性复制全部业务数据。
