...
**这不是 bug，是分布式系统的基本事实。** 必须靠模式来解决，而不是靠"再试一次"。
...
**原因**：数据库行会随重构变化，而事件契约一旦发布就要长期兼容。**两者要解耦。**
...
相关：[[工程知识/后端与分布式系统/分布式可靠性/消息投递与业务提交需要显式的一致性边界]] · [[工程知识/后端与分布式系统/分布式可靠性/一致性模型与共识解决不同问题]] · [[工程知识/数据系统/事务与存储/事务边界应覆盖完整业务用例]]
