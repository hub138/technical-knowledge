...
长任务返回 task ID，由 [[工程知识/后端与分布式系统/分布式可靠性/健康检查只能提供有时效的失败证据]] 的轮询/Webhook/流式机制获取结果。状态接口与 liveness/readiness 分离：任务状态是业务事实，健康检查决定实例是否接流量，心跳只反映最近通信。
...
关联：[[工程知识/计算机系统与性能/存储与网络/端到端网络延迟来自排队、协议、传输与处理]]、[[工程知识/后端与分布式系统/服务设计/流量入口只负责路由与连接，不负责业务正确性]]、[[工程知识/软件构建与质量/测试与交付/测试策略从风险选择证据]]。
