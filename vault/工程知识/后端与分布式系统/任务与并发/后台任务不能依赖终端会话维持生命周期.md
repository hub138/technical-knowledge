...
长任务应建立独立 process group/cgroup，先发送可处理的终止信号，等待宽限期，再强制终止整个组；随后回收子进程、保存退出状态、关闭 fd、完成/回滚临时产物。详细机制见 [[工程知识/后端与分布式系统/任务与并发/任务生命周期必须覆盖进程、日志、超时与清理]]。
...
关联：[[工程知识/AI系统/安全与治理/执行证据必须独立于AI结论]]、[[工程知识/后端与分布式系统/分布式可靠性/租约必须配合FencingToken阻止过期持有者]]、[[工程知识/后端与分布式系统/基础设施/Kubernetes管理资源状态，不管理业务正确性]]。
