...
**关键认知**：这不是"卡不够"，是**显存管理方式不对** + **没有做连续批处理**。换成 vLLM（PagedAttention + 连续批处理）后，同样的 8 卡跑到了 **150 QPS**。
...
PagedAttention 把 KV cache 切成**小块**（类似操作系统的分页）：

```
请求 A（用了 3 个块）    请求 B（用了 1 个块）
  [块1][块2][块7]         [块3]
  ↑ 非连续，按需分配
```
...
- 显存浪费从 60-80% 降到 **不到 4%**
- 支持的并发数提升 **2-4 倍**（这是 vLLM 的核心优势）
- 支持**前缀共享**（多个请求共享相同的 system prompt 前缀）

**注意**：它改善的是**内存管理**，不改变 attention 对历史上下文的语义依赖。

**前缀缓存的坑**：只有在 token 序列**完全匹配**时才命中。多租户场景下要注意隔离——**不要让 A 租户的请求命中 B 租户的前缀**。
