...
> 当前快照：**2026-09-06**。这里放会影响近期工程选择的实现事实；原理和判断方法留在主题页。

本页记录会快速失效、且已经影响工程选择的实现状态。稳定原理留在对应主题页；模型、框架和推理引擎的宣传性能不能直接外推到自己的数据、硬件与流量。

## Agent 与互操作

| 对象 | 截至 2026-09-06 的状态 | 工程动作 |
| --- | --- | --- |
| OpenAI Responses API | 当前新集成主接口 | 记录实际模型快照；Provider 状态不作为唯一业务状态 |
| OpenAI Assistants API | 2026-08-26 已下线 | 删除残留依赖，迁移 Responses/Conversations 或其他运行时 |
...
| OpenAI Agent Builder / Evals | 当前官方文档将其列在 Legacy APIs；具体迁移/下线日期以弃用页为准 | 不把托管产品当作评测核心；保留通用工作流、任务夹具与评测资产 |
| MCP | 发布规范 2026-07-28 | 新实现采用无状态核心；旧 session、初始化与 HTTP+SSE 做迁移 |
...
| 对象 | 当前定位 | 采用时真正要验证 |
| --- | --- | --- |
| vLLM | 以 PagedAttention、连续批处理、前缀缓存和开放模型服务为核心的推理引擎 | 目标模型/精度支持，TTFT/ITL，KV cache、抢占、并行和升级兼容 |
...
推理引擎优化单个模型实例，服务框架处理副本、路由和扩缩，Kubernetes 管理资源放置，平台层负责版本、评测与发布。它们可以组合，但不能用后一层掩盖前一层的容量瓶颈。完整关系见 [[工程知识/AI系统/推理服务与平台/AI平台必须统一模型、数据、评测与发布版本]]。
