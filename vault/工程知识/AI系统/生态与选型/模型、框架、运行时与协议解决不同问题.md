...
模型能力
  ↓ 通过 Provider SDK 暴露
应用组件（提示词、Schema、RAG、工具适配）
  ↓ 交给编排运行时
状态/并发/重试/人工接管/副作用控制
  ↓ 连接外部世界
函数、数据库、MCP、A2A、业务 API
  ↓ 反馈回路
Tracing、评测、成本与安全门禁
```
...
演进的本质不是“越来越自主”，而是把原本藏在 Prompt 或一次性脚本里的假设，逐步显式化为**接口、状态、策略和证据**。每新增一层，都要同时新增可观测性和回退路径。
...
- **MCP**：不是编排框架，而是 Host/Client/Server 之间发现和调用 tools、resources、prompts 的协议；2026-07-28 core 改为无 session 的每请求自描述，但接入后仍要做应用级授权、限流和审计。[历史基线 2025-06-18](https://modelcontextprotocol.io/specification/2025-06-18/index) · [2026-07-28 changelog](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/docs/specification/2026-07-28/changelog.mdx)

选择框架时，先画出自己的状态机和工具契约，再看框架能否表达它；不要先按 GitHub 热度选框架，再被框架的默认抽象反向塑造业务。
...
```text
步骤是否固定、风险是否高？
├─ 是 -> 普通代码/工作流 + 模型做抽取、分类或候选生成
└─ 否 -> 是否需要动态选择工具或根据结果继续？
         ├─ 否 -> 单次模型调用 + schema + 校验
         └─ 是 -> 是否需要长任务、暂停、恢复或并发？
                  ├─ 否 -> 轻量 tool loop / Agent SDK
                  └─ 是 -> 图式编排运行时 + checkpoint + 队列
```

再按约束选择：

- **类型和可测试性优先**：选择 schema-first 的组件或自己保留薄适配层。
- **数据接入复杂**：选择数据/检索框架，但把索引、ACL 和引用契约留在自己的 domain 层。
...
- [[工程知识/AI系统/Agent与工作流/先用工作流，只有路径无法预先枚举时才使用Agent]]
- [[工程知识/AI系统/知识与检索/RAG的核心是选择可引用证据]]
- [[工程知识/AI系统/生态与选型/MCP接入必须验证协议、身份与工具契约]]
...
模型和 SDK 版本、协议草案、价格与弃用信息按 7–30 天复查；运行时可靠性和评测方法按 1–3 个月复查；分层模型与接口原则按 6 个月复查。新事实先进入 [[工程知识/AI系统/生态与选型/AI技术动态]]，通过来源、日期、影响范围和回归结果门禁后再改写本页。
