...
| 实践 | 如何安全接入？ | 版本双栈、contract test、风险分级、审计、超时、熔断和降级 |
...
协议不再替应用保存 session。若工具需要跨调用状态，应返回显式句柄（如 `basket_id`、`browser_id`），后续调用把句柄作为普通参数传回。这样状态有明确所有权、可落库、可审计，也能在普通负载均衡下路由；代价是模型/客户端必须正确传递句柄，应用要处理过期、租户绑定和重放保护。
...
- [ ] 新版 Streamable HTTP POST 请求的 `MCP-Protocol-Version`、`Mcp-Method`、`Mcp-Name` headers 和 `_meta` 均有契约测试；stdio 等其他传输按规范使用对应 `_meta` 字段，旧版握手路径有明确下线日期。
...
同一进程内、数据结构稳定、延迟敏感且不需要跨团队复用的函数，用普通代码更简单。MCP 的收益来自边界和生态，不是“所有功能都必须做成工具”。Agent 的整体编排和风险控制见 [[工程知识/AI系统/Agent与工作流/构建可靠Agent应用]]。
