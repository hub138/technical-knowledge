...
| 类型 | 特征 | 正确策略 | 重试？ |
| --- | --- | --- | --- |
| **瞬时故障** | 超时、503、限流、连接重置 | 退避重试 | ✅ |
| **输入错误** | 参数格式错、类型不对、缺必填 | 修正参数后重试 | ⚠️ 改了才行 |
| **逻辑错误** | 模型选错工具、推理错误 | 换个策略，或给模型反馈让它改 | ❌ 原样重试无效 |
| **环境阻塞** | 权限不足、依赖缺失、网络不通 | **立即停止**，报告需要什么 | ❌ |
| **任务不可完成** | 需求本身矛盾、信息不存在 | **立即停止**，说明原因 | ❌ |
...
- 记录"试过这个，没用"
- 换个角度（换个文件、换个方法）
- 连续 N 次无效 → 停止（见 [[Agent技能/核心机制/Agent循环必须有终止保证]]）
...
**分类必须是代码里的规则，不是让模型判断。** 理由和 [[Agent技能/工具与契约/提示词里的规则会被绕过]] 一样：模型在出错时判断力最差。
...
相关：[[Agent技能/核心机制/Agent循环必须有终止保证]] · [[Agent技能/评测与可靠/成本控制要从上下文和重试两头下手]]
