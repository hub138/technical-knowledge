...
- **可并行**：依赖关系明确后，独立子任务可以并行（见 [[Agent技能/核心机制/多智能体只在能并行或需要独立上下文时才值得]]）
...
见 [[Agent技能/上下文与记忆/记忆要分层不能让模型自己总结]]。计划不是对话历史的一部分，是**结构化状态**。
...
相关：[[Agent技能/核心机制/Agent循环必须有终止保证]] · [[Agent技能/评测与可靠/Agent只能通过轨迹评测]]
