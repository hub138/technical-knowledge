...
同一个"把 1MB 数据从 A 机搬到 B 机"，TCP 栈要经过两侧内核的协议处理与多次拷贝，RDMA（Remote Direct Memory Access，远程直接内存访问）让网卡直接读写对方机器的内存，CPU 与内核全程不参与数据路径。要解决的问题：分布式训练与分布式存储这类机器间大吞吐搬运场景里，TCP 栈的 CPU 开销与拷贝延迟成为瓶颈，RDMA 把这条路径压到微秒级、CPU 占用接近零。
