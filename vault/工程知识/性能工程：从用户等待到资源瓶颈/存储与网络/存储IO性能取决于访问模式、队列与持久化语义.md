...
- [[工程知识/性能工程：从用户等待到资源瓶颈/存储与网络/恢复与重平衡的可用性边界：read_only 与自动恢复的开关语义.md]] —— 存储节点有一组防自伤的降级机制：磁盘满转只读、副本损坏拒绝服务、检测到异常状态停止写入。
- [[工程知识/性能工程：从用户等待到资源瓶颈/存储与网络/存储格式的版本字段是兼容性的锚：写入时必带，读取时逐级识别.md]] —— 落盘格式要有版本锚：写入时必带，读取时逐级识别，否则旧数据在新代码下无法解释
