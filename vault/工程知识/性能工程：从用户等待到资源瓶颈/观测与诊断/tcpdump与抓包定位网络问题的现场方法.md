...
![tcpdump 的真实输出形态：终端里逐包打印时间戳（10:25:35.92…微秒级）、五元组（源地址.端口 > 目标地址.端口）、协议与包长（length 1218），双向流量按到达顺序交错——「把不可见的网络行为变成可指认的证据」就是这种逐行记录](/static/figures/fedoratcpdump.png)
