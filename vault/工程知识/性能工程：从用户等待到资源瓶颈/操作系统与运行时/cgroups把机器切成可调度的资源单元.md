...
```mermaid
graph TD
    A["cpu.max = 0.5 核<br/>每 100ms 周期给 50ms 配额"] -->|"四个业务线程并行跑"| B["四个线程吃满四个核<br/>50ms 配额 12.5ms 就花光"]
    B -->|"配额耗尽"| C["整组节流<br/>剩余 87.5ms 全部挂起"]
    C -->|"下个周期才恢复"| D["p99 延迟毛刺<br/>多线程应用比单线程更陡"]
    B -.->|"单线程视角"| E["只用 0.5 核就不会触发<br/>问题出在并行度与配额的乘积"]
```

毛刺的机制在「并行度乘配额」：0.5 核的配额按 100ms 周期折成 50ms 时间片，JVM 这类多线程进程四个线程并行吃满四个核，50ms 配额 12.5ms 就花光，剩下 87.5ms 整组挂起等下个周期——单核跑满的进程反而没有这个问题。所以容器里的 JVM 要开 UseContainerSupport 让线程池对齐配额，CPU 毛刺排查第一步看 `cpu.stat` 的 `nr_throttled`。
