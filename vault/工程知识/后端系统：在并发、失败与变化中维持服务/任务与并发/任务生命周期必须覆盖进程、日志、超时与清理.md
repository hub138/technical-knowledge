...
editorial_note: "清理生成痕迹2处：最难的不是而是改直接陈述、不仅还改影响范围陈述，命中2到0，密度0.39达标维持1图"
---
...
长任务系统最难的部分，是确保进程、文件描述符、日志、超时和清理在父进程异常时仍有明确归属；启动一条命令只占其中最简单的一段。
...
文件描述符是进程内指向 open file description 的整数句柄；文件、管道、socket、eventfd 都会占用 fd。`fork` 后子进程继承描述符，`exec` 时只有设置 close-on-exec 的描述符会关闭。fd 泄漏的影响超出打开文件本身：pipe、socket、subprocess 会以 `EMFILE` 失败。
