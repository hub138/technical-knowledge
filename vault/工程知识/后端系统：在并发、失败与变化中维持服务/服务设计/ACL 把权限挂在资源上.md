...
与 [[工程知识/后端系统：在并发、失败与变化中维持服务/服务设计/认证授权决定请求能看什么和做什么]] 是同一层的两个视角：那一页拆开认证与授权、比较 RBAC/ABAC/ReBAC；这一页把 ACL 单独讲透。
...
```
请求：主体 S 要对资源 R 执行动作 A
   ↓
1. 取 R 的 ACL 清单
   ↓
2. 在清单上找 S（直接命中 / 通过组间接命中）
   ↓
3. 清单条目是否含 A
   ↓
4. 没有命中条目 → 默认拒绝
...
动作定义得粗，ACL 就表达不了细粒度需求；定义得细，清单条目数量会爆炸。

```
粗：read / write                    → 表达不了"能读但不能列出"
细：read / list / write / delete    → 表达力强，条目变多
```
...
关系：[[工程知识/后端系统：在并发、失败与变化中维持服务/服务设计/认证授权决定请求能看什么和做什么]] · [[工程知识/后端系统：在并发、失败与变化中维持服务/服务设计/API契约定义输入输出和演进边界]] · [[工程知识/缺陷分析：从个案到体系/安全/案例二十六：公告栏没查门禁——Redis 的 ACL 键名泄露与解析崩溃]]
