...
容器网络要解决的问题，是让每个容器拥有"自己的一套网络"：独立的网卡、路由表、iptables 规则，互不干扰，又能按需连通。Linux 的答案是网络命名空间（netns）：内核把网络协议栈整个复制一份（loopback、路由表、邻接表、套接字列表），进程关进哪个 netns 就用哪套协议栈。容器隔离资源边界与镜像身份（见 [[工程知识/后端系统：在并发、失败与变化中维持服务/基础设施/容器隔离资源边界与镜像身份.md]]）给了进程的隔离单元，netns 给了这个单元的网络维度。剩下的问题是：两套互相看不见的协议栈，怎么通信？
...
- **netns 隔离的是网络协议栈，不隔离带宽**。两个容器在同一宿主共享物理网卡，netns 没有任何带宽隔离，一个容器打满网卡，邻居全部卡顿。带宽控制要靠 tc（traffic control）或 Cilium 的带宽管理，隔离单元与网络单元不是一回事。
...
```text
三级连通方案的推演账（通用形态，量级为公开典型值）：

  veth + bridge：同宿主机容器互通走网桥，跨机走物理网
    延迟最低（近裸金属），跨机要自己管 IP 分配
  overlay（VXLAN 类）：跨机容器互通封在隧道里
    报文多封一层 50 字节 → 有效 MTU 降 1450，吞吐税 10-20%
    大报文被分片/丢弃的坑：应用不知情，表现为"偶发超时"
  host 网络：容器直接用宿主机协议栈，零开销，代价是端口冲突与隔离弱化

  排查账：容器里 curl 通、宿主机不通（或反过来）→ 先问自己在哪层命名空间
    MTU 类故障的特征：小包通、大包挂（握手成功、传输卡住）
```

案例回流：MTU 黑洞的实录——overlay 网络下应用偶发卡死，握手正常传输挂起（TLS 证书包大于 1450 被静默丢弃，小包探针全绿）；对齐 overlay 与物理网 MTU 后消失（"小包通大包挂"是 MTU 类故障的指纹）。conntrack 打满的实录——容器高频短连接，宿主机 conntrack 表打满后新建连接被丢，业务报"随机连不上"；调大表容量 + 连接复用后恢复，表水位进监控。跨命名空间排查绕路的实录——网络策略在宿主机 iptables 与容器内各一份，规则冲突时排查花了一天才定位到命名空间层级；排查清单固定"逐层收窄"顺序。隔离的底层边界见 [[工程知识/后端系统：在并发、失败与变化中维持服务/基础设施/容器隔离资源边界与镜像身份.md]]、conntrack 的判定细节见 [[工程知识/性能工程：从用户等待到资源瓶颈/存储与网络/conntrack表满不丢连接的判定：计数、桶与协议差异.md]]、连通性排查的分层纪律见 [[工程知识/性能工程：从用户等待到资源瓶颈/观测与诊断/连通性诊断的分层顺序：从链路到应用逐层收窄，不跳层.md]]。
