...
停止前验证 PID 的命令、启动时间、cwd 和 owner；优先让服务管理器执行 stop。单杀父 PID 可能留下子孙进程，详见 [[工程知识/后端与分布式系统/任务与并发/任务生命周期必须覆盖进程、日志、超时与清理]]。
...
这些命令提供入口，不给出根因。把采样和用户请求、trace、应用队列对齐，再进入 [[工程知识/计算机系统与性能/观测与诊断/系统指标与观测]] 和具体 CPU/内存/I/O/网络页面。
...
关联：[[工程知识/后端与分布式系统/任务与并发/后台任务不能依赖终端会话维持生命周期]]、[[工程知识/计算机系统与性能/观测与诊断/性能问题定位]]、[[工程知识/AI系统/安全与治理/执行证据必须独立于AI结论]]。
