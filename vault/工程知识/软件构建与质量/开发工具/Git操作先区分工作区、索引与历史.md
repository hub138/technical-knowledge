...
`git worktree` 让多个工作目录共享同一 Git 仓库元数据。任务系统必须为 worktree 绑定 owner、revision、task generation 和 lease；其他任务仍在使用时不得 prune/remove 其元数据。每个任务报告实际 `git rev-parse HEAD` 与 dirty state，不能只相信计划中的分支名。见 [[工程知识/AI系统/安全与治理/执行证据必须独立于AI结论]]。
...
关联：[[工程知识/软件构建与质量/软件构建：让变化可以理解、验证与交付]]、[[工程知识/后端与分布式系统/分布式可靠性/租约必须配合FencingToken阻止过期持有者]]、[[工程知识/软件构建与质量/测试与交付/测试策略从风险选择证据]]。
