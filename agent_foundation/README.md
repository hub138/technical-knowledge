...
这是与任何 Agent 框架、模型、CLI 实现、任务格式和应用系统无关的会话、上下文和记忆基础设施。
...
- `context`：RAG 的文档切块、检索协议、RRF 融合、重复片段过滤、上下文预算和引用包。
- `memory`：短期会话窗口、长期记忆记录/协议、过期过滤、时间衰减排序和记忆上下文预算。

## 不包含的东西

- 任意特定 CLI 的参数、认证方式、会话字段名或输出 schema。
- 领域对象、数据库模型、领域相关性定义、评估规则、编排策略或应用状态机。
- 对恢复后“应该继续做什么”的判断；包只保存调用者提供的 continuation。
- embedding 调用、向量库、远程存储、LLM 蒸馏实现或任何凭据配置。
