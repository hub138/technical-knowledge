...
> **这个知识库不收藏结论，只收录能复用的机制。**
>
> - 删掉产品名之后如果只剩口号，说明还没抽出机制。
> - 性能数字必须带测试条件 —— 没有条件的数字不是数字。
> - 来源分 P0–P3：社交媒体只用来发现线索，结论必须落到官方文档、论文或可复现实验。

这几条不是口号，是入库门槛。具体做法见 `vault/知识库管理/方法/一篇知识怎么写.md`
（一页该包含什么：背景、问题、实现思路、效果、错误）和
`vault/知识库管理/方法/知识从哪来怎么更新.md`（来源分级、去重、验证与更新流程）。
...
一级目录只表示稳定知识领域，技能表示对知识点的掌握程度，不建立平行内容分类。同一工程问题只保留一个 canonical 知识点，重复页面先合并案例、机制、代价和验证，再删除旧入口。分类术语见 `CONTEXT.md`，新增内容流程见 `vault/知识库管理/维护/知识点写作与教学输出契约.md`。
